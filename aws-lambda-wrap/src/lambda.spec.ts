...
describe('Lambda', () => {
    ...
    class TestHandler implements LambdaHandler<TestEventData> {
        ...
        public handleEvent(eventData: TestEvent): Promise<any> {
            return undefined;
        }
    }
    ...
    describe('basic usage', () => {
        ...
        describe('handleEvent', () => {
            ...
            it('calls the event transformer with the passed event and context', async () => {
                ...
                expect(transformer.transform).to.have.been
                    .calledOnce
                    .calledWithExactly(event, context);

            });

            it('passes the result of the event transformer to the handler', async () => {

                expect(TestHandler.instance.handleEvent).to.have.been
                    .calledOnce
                    .calledWithExactly(eventData, context);

            });

            it('passes the result of the handler to the responder', () => {

                expect(responder.handleResponse).to.have.been
                    .calledOnce
                    .calledWithExactly(response);
            ...
            describe('errors', () => {
                ...
                it('calls the responder\'s handleError method with the error', () => {
                    ...
                    expect(responder.handleError).to.have.been
                        .calledOnce
    ...
    describe('error handlers', () => {
        ...
        class TestErrorHandler implements LambdaErrorHandler<TestEvent> {
            public handleError(event: TestEvent, error: Error): void {
            }
        ...
        it('calls any error handlers with the error and original event', () => {
            ...
            expect(errorHandler.handleError).to.have.been
                .calledOnce
                .calledWithExactly(event, error);
