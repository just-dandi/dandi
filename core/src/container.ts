...
import {
    ...
    isClassProvider, isFactoryProvider, isGeneratingProvider, isValueProvider,
} from './provider.util';
...
export class Container<TConfig extends ContainerConfig = ContainerConfig> implements Resolver {
    ...
    private async init(): Promise<void> {
        ...
        await Disposable.useAsync(await this.resolve(Scanner, true), async result => {
            ...
            await Promise.all(scanners.map(async (scanner: Scanner) => {
    ...
    private async resolveParam<T>(param: ParamMetadata<T>, token: InjectionToken<T>, optional: boolean, context: ResolverContext<T>) {
    ...
    private async resolveInternal<T>(
        ...
        if (Array.isArray(entry)) {
            return await Promise.all(entry.map(provider => {
    ...
    public async invokeInContext(context: ResolverContext<any>, instance: any, member: Function, ...repositories: Repository[]): Promise<any> {
