...
export class PgDbQueryableBase<TClient extends PgDbQueryableClient> implements DbQueryable {
    ...
    public replaceSelectList<T>(model: Constructor<T>, cmd: string): string {
        ...
        const ogSelect = ogSelectMatch[1]
            .split(',')
        ...
        const joins = joinMatches ?
            joinMatches.map(join => join.match(/join\s+[\w._]+\s+(?:as\s+)?(\w+)/)[1]) :
            [];
        ...
        const matchingDecoratorNames = decoratorSelectList
            .filter(path => aliases.find(alias => path.startsWith(`${alias}.`)));

        if (!matchingDecoratorNames.length) {
            return cmd;
        }

        const newSelect = toKeep.concat(matchingDecoratorNames)
