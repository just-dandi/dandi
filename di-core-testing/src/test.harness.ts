import { Constructor, Disposable } from '../../core/index';
import { Container, InjectionToken, Provider, Repository, ResolverContext, Resolver, ResolveResult } from '../../di-core/index';
...
export interface TestResolver extends Resolver {
    ...
    resolveStub<T>(token: InjectionToken<T>, optional?: boolean, ...repositories: Repository[]):
        Promise<ResolveResult<SinonStubbedInstance<T>>>;

    inject<T>(token: InjectionToken<T>, optional?: boolean, ...repositories: Repository[]):
        Promise<T>;

    injectStub<T>(token: InjectionToken<T>, optional?: boolean, ...repositories: Repository[]):
...
class TestHarness implements TestResolver {
    ...
    public invokeInContext(
        context: ResolverContext<any>, instance: any, member: Function, ...repositories: Repository[]): Promise<any> {
        return this._container.invokeInContext(context, instance, member, ...repositories);
    }

    public resolve<T>(token: InjectionToken<T>, optional?: boolean, ...repositories: Repository[]):
        Promise<ResolveResult<T>> {
        return this._container.resolve(token, optional, ...repositories);
    }

    public resolveStub<T>(token: InjectionToken<T>, optional?: boolean, ...repositories: Repository[]):
        Promise<ResolveResult<SinonStubbedInstance<T>>> {
        return this._container.resolve(
            token, optional, ...repositories) as Promise<ResolveResult<SinonStubbedInstance<T>>>;
    }

    public async inject<T>(token: InjectionToken<T>, optional?: boolean, ...repositories: Repository[]):
        Promise<T> {
    ...
    public async injectStub<T>(token: InjectionToken<T>, optional?: boolean, ...repositories: Repository[]):
        Promise<SinonStubbedInstance<T>> {
