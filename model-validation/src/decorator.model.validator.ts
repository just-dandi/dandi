...
import {
  OneOfValidationAttempt,
  OneOfValidationError,
} from './one.of.validation.error';
...
export class DecoratorModelValidator implements ModelValidator {
  constructor(
    @Inject(TypeValidator(Primitive))
    private primitive: TypeValidator<Primitive<any>>,
  ) {}
  ...
  public validateModel(
    ...
    typeKeys.forEach((key) => {
      ...
      try {
        result[key] = this.validateMember(
          memberMetadata,
  ...
  private validateMetadata(metadata: MemberMetadata, value: any): any {
    ...
    if (
      (!isNaN(metadata.minLength) || !isNaN(metadata.maxLength)) &&
      value.length === undefined
    ) {
      throw new MetadataValidationError(
        'minLength or maxLength',
        'value does not have a length property',
      );
    }
    ...
    if (
      (!isNaN(metadata.minValue) || !isNaN(metadata.maxValue)) &&
      isNaN(value as any)
    ) {
      throw new MetadataValidationError(
  ...
  private validateArrayMember(
    ...
    if (!Array.isArray(value)) {
      throw new ModelValidationError(
        key,
        new TypeValidationError(value, Array),
      );
    }
    ...
    return value.map((entry, index) => {
      ...
      return this.validateMember(
        entryMeta,
        this.getKey(key, index.toString()),
        entry,
      );
    });
  }
