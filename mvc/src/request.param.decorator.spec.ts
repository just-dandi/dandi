...
describe('@RequestParam', () => {
  it('sets a token for the decorated parameter', () => {
    class TestController {
      public method(
        @PathParam(String) foo: string,
        @QueryParam(String) bar: string,
      ): void {}
    }

    const pathMeta = getInjectableParamMetadata(
      methodTarget(TestController),
      'method',
      0,
    );
    const queryMeta = getInjectableParamMetadata(
      methodTarget(TestController),
      'method',
      1,
    );
    ...
    expect(pathMeta.token.toString()).to.contain(
      `@dandi/mvc#${RequestPathParamMap}:method:foo`,
    );

    expect(queryMeta).to.exist;
    expect(queryMeta.token).to.exist;
    expect(queryMeta.token).to.be.instanceOf(SymbolToken);
    expect(queryMeta.token.toString()).to.contain(
      `@dandi/mvc#${RequestQueryParamMap}:method:bar`,
    );
  });
  ...
  it('creates a provider to handle validating the param value', () => {
    ...
    expect(
      (pathMeta.providers[0] as FactoryProvider<any>).deps,
    ...
    expect(
      (queryMeta.providers[0] as FactoryProvider<any>).deps,
