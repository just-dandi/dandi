...
describe('PgDbClient', function () {
  ...
  describe('transaction', function () {
    ...
    it('creates multiple transaction clients in a way to allows one to fail without affecting the other', async function () {
      ...
      const txn1 = this.dbClient.transaction(async (txnClient) => {
        onTxnClient1(txnClient)
        await ready
        throw new Error('oh nooo')
      })
      const txn2 = this.dbClient.transaction(async (txnClient) => {
        onTxnClient2(txnClient)
        await ready
        return 'hi!'
      })
  ...
  describe('query', function () {
    ...
    it('calls through to baseQuery', async function () {
      ...
      expect(this.dbClient.baseQuery).to.have.been.calledOnce.calledWithExactly(
        this.pool,
  ...
  describe('queryModel', function () {
    ...
    it('calls through to baseQueryModel', async function () {
      ...
      expect(this.dbClient.baseQueryModel).to.have.been.calledOnce.calledWithExactly(
        this.pool,
        TestModel,
        'SELECT foo FROM bar WHERE id = $1',
        [1],
  ...
  describe('queryModelSingle', function () {
    ...
    it('calls through to baseQueryModelSingle', async function () {
      ...
      expect(this.dbClient.baseQueryModelSingle).to.have.been.calledOnce.calledWithExactly(
        this.pool,
        TestModel,
        'SELECT foo FROM bar WHERE id = $1',
        [1],
