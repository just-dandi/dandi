...
import { RootInjectorContext } from '@dandi/core/testing'
...
describe('ResolverContext', () => {
  ...
  let parentResolverContext: ResolverContext
  let childResolverContext: ResolverContext
  ...
  beforeEach(() => {
    ...
    parentContext = new InjectorContext(rootInjectorContext, function TestParentContext(){}, [parentProvider1, parentProvider2, sharedProviderParent])
    parentResolverContext = new ResolverContext(parentToken1, parentContext)
    childContext = parentContext.createChild(function TestChildContext(){}, childProvider1, childProvider2, sharedProviderChild)
    childResolverContext = new ResolverContext(childToken1, childContext)
  })
  ...
  describe('match', () => {
    it('returns undefined if no token is found', () => {
      expect(new ResolverContext(new SymbolToken('test'), rootInjectorContext).match).to.be.undefined
    })
    ...
    it('returns the first matching entry when the same token is defined in a child scope', () => {
      ...
      const overridingContext = new ResolverContext(sharedToken, childContext)
