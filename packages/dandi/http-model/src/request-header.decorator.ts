...
import { HttpRequestHeader, HttpRequestHeaders, requestHeaderProvider, requestHeaderToken } from '@dandi/http'

export interface RequestHeader<THeaderName extends HttpRequestHeader>
  extends ParamMetadata<HttpRequestHeaders[THeaderName]> {
...
export function requestHeaderDecorator<THeaderName extends HttpRequestHeader>(
  ...
  const meta = getInjectableParamMetadata<HttpRequestHeaders[THeaderName], RequestHeader<THeaderName>>(
    target,
    propertyName,
    paramIndex,
  )
...
export function RequestHeader<THeaderName extends HttpRequestHeader, TTarget>(
  headerName: THeaderName,
): ParameterDecorator {
