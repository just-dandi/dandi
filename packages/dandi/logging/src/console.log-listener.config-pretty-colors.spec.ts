...
import { ColorsNotLoadedError, initColors, PRETTY_COLORS_CONTEXT_TAG, PRETTY_COLORS_LEVEL_TAG } from '@dandi/logging'
...
describe('PrettyColorsLogging', function () {
  describe('no colors', function () {
    ...
    it('throws an error when the colors package is not available', function () {
      expect(() => PRETTY_COLORS_CONTEXT_TAG({ level: LogLevel.debug } as any)).to.throw(ColorsNotLoadedError)
    })
  })
  ...
  describe('with colors', function () {
    ...
    describe('contextTag', function () {
      it('uses the level-themed color for formatting the context tag', function () {
        ...
        expect(colors['debug']).to.have.been.calledOnce.calledWith('test')
      })
    })
    ...
    describe('levelTag', function () {
      it('uses the level-themed color for formatting the context tag', function () {
        ...
        expect(colors['debug']).to.have.been.calledOnce.calledWith('DEBUG')
      })
    })
  })
})
